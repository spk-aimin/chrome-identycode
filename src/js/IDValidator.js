...
;(function(factory){
    ...
    // CommonJS
     ...
     _IDValidator.prototype = {
        ...
        //仿造一个号 {addr: "", birth: "", gender: ""}
        makeIDByCondition:function(opts, isFifteen){
                var GB2260 = this.GB2260 || null;
                opts = opts || {};
                var tyr, tmo, tda;
                if(opts.birth) {
                    var tempDate = new Date(opts.birth);
                    tyr =""+ tempDate.getFullYear();
                    tmo =""+ (tempDate.getMonth() + 1);
                    tda =""+ tempDate.getDate();
                }
                //地址码
                var addr = opts.addr ||null;
                if( GB2260 !== null ){
                    var loopCnt = 0;
                    while( addr === null ){
                        //防止死循环
                ...
                //出生年
                var yr =tyr || '19' + util.str_pad( util.rand( 99,50 ), 2, '0' );
                var mo =tmo?util.str_pad(tmo, 2, '0' ) : util.str_pad( util.rand( 12,1 ), 2, '0' );
                var da =tda?util.str_pad(tda, 2, '0' ): util.str_pad( util.rand( 28,1 ), 2, '0' );
                if( isFifteen ){
                    return addr + yr.slice(2) + mo + da + util.str_pad( util.rand( 999,1, opts.gender ),3,'1' );
                }
   
                var body = addr + yr + mo + da + util.str_pad( util.rand( 999,1, opts.gender ),3,'1' );
